...
Public Class Form10
    ...
    Private Sub FilterPesanan(keyword As String)
        ...
        If keyword <> "" AndAlso Not hasVisiblePanel Then
            If lblNotFound Is Nothing Then
                ...
                flpRiwayat.Controls.Add(lblNotFound)
            End If
        ElseIf lblNotFound IsNot Nothing Then
            flpRiwayat.Controls.Remove(lblNotFound)
            lblNotFound.Dispose()
        End If
    End Sub

    Private Sub LoadDataRiwayatPesanan()
        ...
        Try
            ...
            Dim query As String = "SELECT td.transaksi_detail_id, t.user_id, u.nama AS nama_user, " &
                      ...
                      "WHERE td.status = 'Diselesaikan' " &  ' Tambahkan filter untuk Ditolak
            ...
            While reader.Read()
                ...
                CreatePesananCardRiwayat(pesanan, namaUser)
            End While

            If recordCount = 0 Then
                Dim lblEmpty As New Label()
                lblEmpty.Text = "Tidak ada riwayat pesanan (Disetujui/Ditolak)"
                lblEmpty.Font = New Font("Arial", 12, FontStyle.Italic)
                lblEmpty.ForeColor = Color.Gray
                lblEmpty.AutoSize = True
                flpRiwayat.Controls.Add(lblEmpty)
            End If

            reader.Close()
        Catch ex As Exception
            MessageBox.Show("Gagal memuat data riwayat pesanan: " & ex.Message, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error)
        Finally
            TutupKoneksi()
        End Try
    ...
    Private Sub CreatePesananCardRiwayat(pesanan As Pesanan, namaUser As String)
        Dim cardPanel As New Panel()
        With cardPanel
            .Width = 510
            .Height = 170
            .BackColor = If(pesanan.Status = "Ditolak", Color.FromArgb(255, 235, 235), Color.FromArgb(228, 228, 210)) ' Warna merah muda untuk yang ditolak
        ...
        With lblInfo
            ...
            $"Tanggal: {pesanan.TanggalPesan.ToString("dd/MM/yyyy HH:mm")}" & vbCrLf &
            $"Status: {pesanan.Status}"
            .Font = New Font("Bell MT", 10)
            .Top = 15
            .Left = 150
            .AutoSize = True
            .ForeColor = If(pesanan.Status = "Ditolak", Color.FromArgb(200, 0, 0), Color.FromArgb(64, 64, 64)) ' Teks merah untuk Ditolak
        ...
        flpRiwayat.Controls.Add(cardPanel)
    End Sub
    ...
    Private Function CreateDefaultImage(width As Integer, height As Integer) As Image
        ...
        Using g As Graphics = Graphics.FromImage(bmp)
            ...
            Using font As New Font("Bell MT", 8)
    ...
    Private Sub btnDaftarPesanan_Click(sender As Object, e As EventArgs) Handles btnDaftarPesanan.Click
        AnimateTransition(Form9)
    End Sub

    Private Sub Button_Hover(sender As Object, e As EventArgs) Handles _
    btnTambahBunga.MouseEnter, btnDaftarPesanan.MouseEnter, btnRiwayatPesanan.MouseEnter, btnLogout.MouseEnter
    ...
    btnTambahBunga.MouseLeave, btnDaftarPesanan.MouseLeave, btnRiwayatPesanan.MouseLeave, btnLogout.MouseLeave
